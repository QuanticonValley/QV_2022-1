...
const Login = () => {
  ...
  async function showAuthInfo() {
	if (window.isAuthenticated) { 
		await setTimeout(document.getElementById("authenticated").style.setProperty('display', 'none'), 3000);
		
		 
	} else {
		await setTimeout(() =>{document.getElementById("authenticated").style.removeProperty('display')
	console.log("is not authenticated")}, 3000);
