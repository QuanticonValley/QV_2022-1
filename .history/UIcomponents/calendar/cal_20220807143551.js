...
const EventDot = ({info, expires}) => {
   if (expires) {
      return(<div className='evenDot' style={{background: 'red'}}  />)
   }
   else  return(<div className='evenDot' />)}


const Day = ({ d, y, m, haClick, event, today, type }) => {
      ...
      return (
         <li
            ...
            style={{
               ...
               ...liStyle
            }}> {d} {event ? <EventDot  type={type}/> : null}
            {console.log(type)}
         </li>
      )
   }
   ...
   return <li onClick={haClick} style={liStyle}>
      {d} {event ? <EventDot info={info} expires ={expires}/> : null}
   </li>
}
            ...
            {weekDays.map(d => <WeekDay key={d} d={d} /> )}
            {
               days.map(d =>
                  <Day
                     ...
                     type={events.some(e => e.d === d && e.m === month + 1  && e.y === year &&  e.info === undefined)}
