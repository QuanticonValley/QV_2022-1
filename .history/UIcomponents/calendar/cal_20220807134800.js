...
const EventDot = () => <div className='evenDot'  />

const Day = ({ d, y, m, haClick, event, today, isInfo }) => {
...
const Calendar = ({ styles, handleDay, events }) => {
   ...
   console.log(events.some(e => e.d === d && e.m === month + 1 && e.y === year && e.info != undefined))
            ...
            {weekDays.map(d => <WeekDay key={d} d={d} />)}
            {
               days.map(d =>
                  <Day
                     ...
                     isInfo={events.some(e => e.d === d && e.m === month + 1 && e.y === year && e.info != undefined)}
